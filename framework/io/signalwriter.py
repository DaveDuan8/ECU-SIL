"""
framework/io/signalwriter
-------------------

Signal Writer Class

:org:           Continental AG
:author:        Ralf Leidenberger

:version:       $Revision: 1.2 $
:contact:       $Author: Leidenberger, Ralf (uidq7596) $ (last change)
:date:          $Date: 2020/03/31 09:13:03CEST $
"""
...
class CsvWriter(object):
    ...
    def __init__(self, fp, **xargs):
        ...
        if not hasattr(self._fp, 'write'):
            self._self_open = True
            try:
                self._fp = open(str(self._fp), "wb")
            except:
                raise SignalWriterException("Error while trying to open file, corrupted?")
        else:
            self._self_open = False
...
class SignalWriter(object):
    ...
    *Examples*
    ...
    .. code-block:: python
        ...
        import numpy as np
        from framework.io.signalwriter import SignalWriter, SignalWriterException
...
-----------
$Log: signalwriter.py  $
Revision 1.2 2020/03/31 09:13:03CEST Leidenberger, Ralf (uidq7596) 
initial update
Revision 1.1 2020/03/25 21:12:08CET Leidenberger, Ralf (uidq7596) 
Initial revision
